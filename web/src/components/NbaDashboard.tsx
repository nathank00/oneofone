...
import type { GameLog, WLRecord } from "@/lib/types";
import DatePicker from "./DatePicker";
import PredictionCard from "./PredictionCard";
import RecordBadge from "./RecordBadge";

/** Only fetch the columns we need — gamelogs has 70+ columns */
const SELECT_COLS = [
  "GAME_ID",
  "GAME_DATE",
  "AWAY_NAME",
...
export default function NbaDashboard() {
  ...
  const fetchGames = useCallback(async (dateStr: string) => {
    ...
    if (error) {
      ...
      setGames((data as unknown as GameLog[]) ?? []);
    }
    setLoading(false);
  }, []);
  ...
  const fetchAllTimeRecord = useCallback(async () => {
    ...
    // Supabase JS can't do column-to-column comparison, so fetch both
    // columns where both are non-null and compute client-side.
    // With ~5k games this is a tiny payload (~50KB).
    const { data, error } = await supabase
      .from("gamelogs")
    ...
    if (error || !data) {
      console.error("Error fetching all-time record:", error);
