...
import { getPortfolioOverview } from "../lib/commands";
import type { PortfolioOverview, Tab } from "../lib/types";
...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const data = await withTimeout(getPortfolioOverview(), 10000);
      setPortfolio(data);
    } catch (e) {
      setPortfolio({
        connected: false,
  ...
  if (!portfolio || !portfolio.connected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-6">
        <div className="rounded-full bg-neutral-800/50 p-4 mb-4">
          <svg className="w-8 h-8 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M13.19 8.688a4.5 4.5 0 011.242 7.244l-4.5 4.5a4.5 4.5 0 01-6.364-6.364l1.757-1.757m9.86-2.813a4.5 4.5 0 00-6.364-6.364L4.5 8.25" />
          </svg>
        </div>
        <h2 className="text-lg font-semibold text-neutral-300 mb-2">Not Connected</h2>
        <p className="text-sm text-neutral-500 mb-1 max-w-xs">
          {portfolio?.error || "Configure your Kalshi API key to get started."}
        </p>
        <button
          ...
          {portfolio.positions.length === 0 ? (
            ...
            <div className="divide-y divide-neutral-800/60">
              ...
              {portfolio.positions.map((pos) => (
                ...
                >
                  <div className="font-mono text-xs text-neutral-200 truncate" title={pos.ticker}>
                    {pos.ticker}
                  </div>
