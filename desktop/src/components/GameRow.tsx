...
import { placeBet } from "../lib/commands";
...
/** Compute contract count from settings + price, mirroring the Rust logic */
function computeContracts(
  ...
  if (sizingMode === "contracts") {
    return Math.max(1, Math.floor(betAmount));
  }
  // dollars mode: floor(budget / price), min 1
  if (price <= 0) return 1;
  return Math.max(1, Math.floor(betAmount / price));
}

export default function GameRow({ game, sizingMode, betAmount }: GameRowProps) {
  ...
  const price =
    game.betSide === "yes" ? game.yesAsk : game.noAsk;

  const contractCount = price
    ? computeContracts(inputMode, inputAmount, price)
    : 0;
    ...
    <div className="rounded-lg border border-neutral-800 bg-neutral-900/60 p-4 transition-colors hover:border-neutral-700">
      ...
      <div className="mt-2 flex items-center gap-4 text-sm">
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wider text-neutral-600">Pick</span>
          <span className="font-semibold text-white">
            {game.predictedWinner.split(" ").pop()}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wider text-neutral-600">Model</span>
          <span className="font-mono text-white">
            {Math.round(game.modelProb * 100)}%
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wider text-neutral-600">Market</span>
          <span className="font-mono text-neutral-400">
            {Math.round(game.marketImpliedProb * 100)}%
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-wider text-neutral-600">Edge</span>
        ...
        <div className="ml-auto">
          {!showBetForm ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                onClick={() => setInputMode(isContracts ? "dollars" : "contracts")}
